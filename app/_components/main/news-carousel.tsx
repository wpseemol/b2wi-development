                    ...
                    <ul className="whitespace-no-wrap flex-no-wrap [backface-visibility: hidden] [transform-style: preserve-3d] [touch-action: pan-Y] [will-change: transform] relative flex w-full overflow-hidden p-0">
                        {newData.map((news) => (
                            <li key={news.id} className="group">
                                <figure className="w-full sm:h-[240px] h-[200px] rounded-lg">
                                    <Image
                                        width={350}
                                        height={240}
                                        alt={news.title}
                                ...
                                <div className="ml-1 mt-3">
                                    <div className="flex items-center gap-1 text-xs mb-3 text-neutral-700/70">
                                        <FaTags /> <h3>{news.publishedBy}</h3>
                                    </div>
                                    <p className="text-primaryColor/70 text-sm mb-1">
                                        {news.publishedAt}
                                    </p>
                                    <h2 className="text-base font-semibold text-[#002c4b] group-hover:underline duration-150">
                                        {news.title}
                                    </h2>
                                    <button className="mt-4 text-base text-neutral-700/75 flex items-center gap-1 group-hover:gap-3 duration-150">
                                        Read More{' '}
                                        <span className="">{'>'}</span>
                                    </button>
                                </div>
